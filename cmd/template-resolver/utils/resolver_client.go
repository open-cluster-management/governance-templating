...
func (t *TemplateResolver) GetCmd() *cobra.Command {
  ...
  is a dash ('-') or absent, template-resolver reads from the standard input.`,
	...
	templateResolverCmd.Flags().StringVar(
		...
		"object-namespace",
		"",
		"the object namespace to use for the .ObjectNamespace template variable when policy use namespaceSelector",
	)
	templateResolverCmd.Flags().StringVar(
		&t.objName,
		"object-name",
		"",
		"the object namespace to use for the .ObjectNamespace template variable "+
			"when policy use namespaceSelector or objectSelector",
	)
